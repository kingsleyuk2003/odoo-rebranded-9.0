...
def configure_parser():
    ...
    group = optparse.OptionGroup(parser, "Note",
        ...
        "with the System server for case management in the CRM module.")
    parser.add_option_group(group)
    parser.add_option("-u", "--user", dest="userid",
                      help="System user id to connect with",
    ...
    parser.add_option("-d", "--dbname", dest="dbname",
                      help="System database name (default: %default)",
                      default=config.OPENERP_DEFAULT_DATABASE)
    parser.add_option("--host", dest="host",
                      help="System Server hostname",
                      default=config.OPENERP_HOSTNAME)
    parser.add_option("--port", dest="port",
                      help="System Server XML-RPC port number",
...
def main():
    ...
    try:
        ...
        subject = '[System]:ERROR: Mailgateway - %s' % time.strftime('%Y-%m-%d %H:%M:%S')
        send_mail(
            config.MAIL_ERROR,
            config.MAIL_ADMINS,
            subject, msg, files=[('message.txt', msg_txt)]
        )
        sys.stderr.write("Failed to deliver email to the System Server, sending error notification to %s\n" % config.MAIL_ADMINS)
